...
const db = createClient(
  "https://ljjzgddqbwpervtablgj.supabase.co",
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imxqanpnb..."
);

// NOTE: full anon key below (split to avoid PS heredoc issues)
const ANON = [
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9",
  "eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imxqanpn",
  "ZGRxYndwZXJ2dGFibGdqIiwicm9sZSI6ImFub24i",
  "LCJpYXQiOjE3ODk1NTY0NzgsImV4cCI6MjEwNTEz",
  "MjQ3OH0.4FDByk8iEv3RWpx6t2dj7sgLn3DUqL3HQh-tvX0LPss"
].join("");

// re-init with correct key
const DB = createClient("https://ljjzgddqbwpervtablgj.supabase.co", ANON);
...
// ── AUTO-RESTORE: CLI session bridge ────────────────
// Try to grab the active CLI session from Flask /api/session.
// If Flask isn't running (e.g. opened standalone), fall back to localStorage.
(async function restoreSession(){
  try {
    const res = await fetch('/api/session', { cache: 'no-store' });
    if (res.ok) {
      const cli = await res.json();
      // CLI returned a valid session
      if (cli && cli.username) {
        applySession({
          id:          cli.id,
          username:    cli.username,
          role:        cli.role,
          token_quota: cli.token_quota || 0,
          token_used:  cli.token_used  || 0,
        });
        go('create-mod');
        toast(`Auto-login dari CLI: ${cli.username}`, 'ok');
        return;
      }
    }
  } catch (_) { /* Flask not running — fall through */ }
